...
const StudentModal = ({ student, role, onClose }) => {
  ...
  const handleSubmit = () => {
    ...
    if (student.id) {
      // If the student has an ID, update the student
      ...
      // If the student doesn't have an ID, create a new student
      createStudent(studentData)
        .then((data) => {
          // Update the student ID with the response data
          student.id = data.id;
          return Promise.all(
            family.map((f) =>
