    ...
    >
      ...
      <div className="flex flex-col gap-4 mb-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          It looks like the issue is that the `nationality` state is not being
          updated when selecting a new nationality from the dropdown. The
          `onChange` handler for the select element is calling
          `handleNationalityChange`, but that function is likely not updating
          the `nationality` state. To allow changing the nationality,
          `handleNationalityChange` needs to update the `nationality` state. For
          example: ```jsx const handleNationalityChange = (e) =>{" "}
          {setNationality(nationalities.find((n) => n.id === e.target.value))}
          ``` This will find the nationality object that matches the selected id
          and update the `nationality` state with it. So in summary: -
          `onChange` calls `handleNationalityChange` - But
          `handleNationalityChange` is not updating `nationality` state - Need
          to update `nationality` state in `handleNationalityChange` to allow
          changing value Let me know if this helps explain why the nationality
          gets stuck at the initial value!
        </div>
      </div>
      <div className="flex flex-col gap-4 mb-4">
