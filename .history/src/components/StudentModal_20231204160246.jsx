...
const StudentModal = ({ student, role, onClose }) => {
  ...
  const handleFamilyFirstNameChange = (index, e) => {
    const updatedFamily = [...family];
    updatedFamily[index].firstName = e.target.value;
    setFamily(updatedFamily);
  };
  
  const handleFamilyLastNameChange = (index, e) => {
    const updatedFamily = [...family];
    updatedFamily[index].lastName = e.target.value;
    setFamily(updatedFamily);
  };
  
  const handleFamilyDateOfBirthChange = (index, date) => {
    const updatedFamily = [...family];
    updatedFamily[index].dateOfBirth = date.toISOString();
    setFamily(updatedFamily);
  };
      ...
      <div className="flex flex-col gap-4 mb-4">
        ...
        {family.map((f, i) => (
          <div key={i} className="flex flex-col gap-2 border rounded p-4">
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor={`familyFirstName${i}`}>First Name</label>
              <input
                id={`familyFirstName${i}`}
                type="text"
                className="border rounded px-2 py-1"
                value={f.firstName}
                onChange={(e) => handleFamilyFirstNameChange(i, e)}
                disabled={role === "Admin" && student.approved}
              />
            </div>
            <div className="flex flex-col gap-2">
              <label htmlFor={`familyLastName${i}`}>Last Name</label>
              <input
                id={`familyLastName${i}`}
                type="text"
                className="border rounded px-2 py-1"
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor={`familyDateOfBirth${i}`}>Date of Birth</label>
              <DatePicker
                id={`familyDateOfBirth${i}`}
                selected={f.dateOfBirth}
                onChange={(date) => handleFamilyDateOfBirthChange(i, date)}
                dateFormat="yyyy-MM-dd"
                className="border rounded px-2 py-1 bg-gray-100"
                disabled={role === "Admin" && student.approved}
              />
