...
const LandingPage = () => {
  ...
  useEffect(() => {
    Promise.all(students.map(student => fetchStudentNationality(student.id).catch(() => null)))
      .then(nationalities => students.map((student, index) => ({ ...student, nationality: nationalities[index] })))
