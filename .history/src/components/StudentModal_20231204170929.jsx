...
const StudentModal = ({ student, role, onClose }) => {
  ...
  const [familyFirstName, setFamilyFirstName] = useState(student.lastName);
  const [familyLastName, setFamilyLastName] = useState(student.lastName);
  ...
  const handleFamilyFirstNameChange = (index, e) => {
    setFamily((prevFamily) =>
      prevFamily.map((f, i) =>
        i === index ? { ...f, familyFirstName: e.target.value } : f
      )
    );
  };
  const handleFamilyLastNameChange = (index, e) => {
    setFamily((prevFamily) =>
      prevFamily.map((f, i) =>
        i === index ? { ...f, familyLastName: e.target.value } : f
      )
    );
  };
      ...
      <div className="flex flex-col gap-4 mb-4">
        ...
        <div key={i} className="flex flex-col gap-2 border rounded p-4">
          <div className="flex justify-between items-center">
            <span className="text-lg font-medium">Family Member #{i + 1}</span>
            <button
